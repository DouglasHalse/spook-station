...
emfReaderConnectionState = deviceManager.devices["EMFReader1"].getConnectionState()
...
for desiredState in range(0, 5):
    deviceManager.devices["EMFReader1"].setDesiredState(desiredState)
    time.sleep(1)
    currentState = deviceManager.devices["EMFReader1"].getCurrentState()
...
for desiredUseSound in [True, False]:
    deviceManager.devices["EMFReader1"].setDesiredUseSound(desiredUseSound)
    time.sleep(1)
    currentUseSound = deviceManager.devices["EMFReader1"].getCurrentUseSound()
...
deviceManager.devices["EMFReader1"].setFluctuationRate(1)
deviceManager.devices["EMFReader1"].setDesiredState(desiredState)
time.sleep(1)
currentState = deviceManager.devices["EMFReader1"].getCurrentState()
if (currentState != 2):
...
for fluctuationMagnitude in range(0, 3):
    deviceManager.devices["EMFReader1"].setFluctuationMagnitude(fluctuationMagnitude)
    time.sleep(1)
